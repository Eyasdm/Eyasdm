...
Desktop application built for a real client (Al-Khattaf boutique). Electron + React frontend, SQLite for local-first storage, bidirectional multi-device sync engine with conflict resolution via Supabase, cloud backup on Cloudflare R2. Handles invoicing, inventory, accounting ledgers, and role-based access. RTL/Arabic UI. Running in a live business.
